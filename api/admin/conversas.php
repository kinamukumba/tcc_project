...
// Buscar lista de utentes que trocaram mensagens com o admin
$query = "SELECT DISTINCT u.id_usuario, u.nome, u.email 
          ...
          WHERE u.tipo_usuario = 'utente' AND (m.remetente_id = :id_admin OR m.destinatario_id = :id_admin)
...
foreach($conversas as &$c) {
    $qMsg = "SELECT conteudo, data_envio FROM mensagens 
             WHERE (remetente_id = :u AND destinatario_id = :a) 
                OR (remetente_id = :a AND destinatario_id = :u)
             ORDER BY data_envio DESC LIMIT 1";
    $sMsg = $db->prepare($qMsg);
    $sMsg->bindParam(':u', $c['id_usuario']);
    $sMsg->bindParam(':a', $id_admin);
    $sMsg->execute();
