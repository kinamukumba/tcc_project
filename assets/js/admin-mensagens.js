...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Atualiza lista de conversas e mensagens a cada 5 segundos
    setInterval(() => {
        loadConversations();
    ...
    // Verificar se há um ID de usuário na URL para iniciar conversa
    const urlParams = new URLSearchParams(window.location.search);
    const userIdParam = urlParams.get('id');
    if(userIdParam) {
        currentChatUserId = userIdParam;
        // Precisamos buscar o nome do usuário se ele não estiver na lista de conversas ativas
        fetch(`/tcc_project/api/admin/utentes.php?id=${userIdParam}`)
            .then(r => r.json())
            .then(user => {
                if(user) selectConversation(user.id_usuario, user.nome);
            });
    }
...
async function loadConversations() {
    try {
        const response = await fetch('../api/admin/conversas.php');
...
async function selectConversation(userId, userName) {
    ...
    // Se a função foi disparada por um clique, marca o item
...
async function loadMessages(userId) {
    try {
        // Usamos o chat.php genérico, mas filtrando
        ...
        if (response.ok) {
            // Filtrar apenas mensagens entre admin e este utente
            // Nota: O chat.php GET atual pega TUDO do usuário logado. 
            // Precisamos atualizar o chat.php para filtrar por destinatário se fornecido.
            data.forEach(msg => {
